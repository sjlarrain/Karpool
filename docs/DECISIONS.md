...
| D-18 | Kudos prompt has no "no thanks" path — the sketch's rate overlay has a 💚 toggle and submits either way; the built version only offers "Send kudos", so a rider who doesn't want to give kudos can never clear the prompt off a closed trip | Record the decline (a `kudos.declined` flag or a dismissed-at column) rather than hiding it client-side, so the prompt stays cleared across devices — but this is a product call, not an engineering one | Open — surfaced by the 2026-08-19 sketch-vs-app sweep. Blocks finishing the kudos flow. |
